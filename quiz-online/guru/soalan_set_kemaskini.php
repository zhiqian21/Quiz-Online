...
if(!empty($_POST))
{
	...
	# arahan untuk mengemaskini data set_soalan
    ...
    $result=mysqli_query($condb,$arahan_kemaskini);
...
<!-- bahagian untuk senarai set soalan_set -->
...
<table width='100%' border='1' id='besar'>
	...
	<td>
		<textarea name='topik' rows="4" cols="25" >
		<?PHP echo $_GET['topik']; ?></textarea>
	</td>
	<td>
		<textarea name='arahan' rows="4" cols="25" >
	...
	<select name='jenis'>
		...
		<option vale='Kuiz'>Kuiz</option>
	</select>
	</td>
		<td><input type='date' name='tarikh' value='<?PHP echo $_GET['tarikh']; ?>'></td>
		<td><input type='number' name='masa' value='<?PHP echo $_GET['masa']; ?>'></td>
	...
	# Memaparkan data yang diambil baris demi baris
		echo "<tr>
			...
			|<a href='padam.php?jadual=set_soalan&medan=no_set&kp=".$data['no_set']."' onClick=\"return confirm('Anda pasti anda ingin memadam data ini.')\" >Padam</a>|
